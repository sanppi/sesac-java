...
// 예외가 발생 시 프로그램의 갑작스런 종료를 막고 정상 실행할 수 있도록 처리하는 코드를 예외 처리 코드라고 함
// try-catch-finally 블록으로 처리

// try 블록에서 작성한 코드가 예외 없이 정상 실행 -> catch 실행 x, finally 실행 o
// try 블록에서 작성한 코드에서 예외 발생 -> catch 실행 0, finally 실행 o
// 심지어 try 블록과 catch 블록에서 return 문을 사용해도 finally 는 항상 실행됨 (finally 는 생략 가능)
...
//        System.out.println(getValueByIdx(numbers, numbers.length)); // ArrayIndexOutOfBoundsException
        ...
        try {
            ...
            // 배열 인덱싱 중 에러 발생 >> java.lang.ArrayIndexOutOfBoundsException: Index 5 out of bounds for length 5
        }
